 ...
 * presentation of the model and the forwarding of the user interaction to its
...
public interface DrawView {
	 ...
	 * <code>MouseEvent e</code>, a pair of coordinates is given to this procedure.
	 * <code>x</code> and <code>y</code> represent the constrained position of
	 * <code>e.getX()</code> and <code>e.getY()</code>.
	 * 
	 * @param x the constrained x-coordinate of the mouse.
	 * @param y the constrained y-coordinate of the mouse.
	 * @param e the mouse event
	 * @return the handle at the mouse position or <code>null</code> if no handle is
	 *         selected.
	 */
	FigureHandle getHandle(int x, int y, MouseEvent e);

	/**
	 * Returns a list which contains the currently selected figures. The figures are
	 * ordered in the order in which they were selected by the user. If no figures
	 * are selected, then an empty list is returned.
	 * 
	 * Changes applied to this list do not change the selection state of any figure.
	 * For changing the selection see methods {@link #addToSelection(Figure)
	 * addToSelection}, {@link #removeFromSelection(Figure) removeFromSelction} and
	...
	/**
	 * Adds a figure to the selected figures. The figure is added at the end of the
	...
	/**
	 * Sets a selection rubber band to be displayed by the draw view. If the
	 * <code>rect</code> parameter passed is <code>null</code> then new rubber band
	 * is displayed.
	 * 
	...
	/**
	 * Draws the graphic model. The paint method iterates over all figures in the
	 * graphic model and calls their paint method.
	 * 
