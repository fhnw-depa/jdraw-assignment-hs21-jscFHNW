...
/**
 * The interface DrawContext represents the context which contains and presents
 * the draw view. It contains methods to access the contained view and a
 * convenience method to access the associated model.
 * 
 * DrawContext provides methods to communicate with the context, e.g. in order
 * to display status messages and to add Menu Actions and tool factories.
 * 
 * @see DrawView
 * @see DrawModel
 *
...
public interface DrawContext {
	...
	/**
	 * Adds a draw tool to the editor's capabilities. An editor has a list of tools
	 * that are available to the user. This list must guarantee that any tool is
