...
/**
 * Standard implementation of interface DrawContext. This implementation uses a
 * JFrame as container element. It provides common initialization and behaviour
 * for all implementations based on a JFrame.
 * 
 * @see DrawView
...
public abstract class AbstractContext extends JFrame implements DrawContext {
	...
	/**
	 * Constructs a standard context by creating and initializing a GUI. A default
	 * set of DrawTools is used.
	 * 
	...
	/**
	 * Constructs a standard context by creating and initializing a GUI. The drawing
	 * tools available can be parameterized using <code>toolFactories</code>.
	 * 
	 * @param view          the view that displays the figures within the editor.
	...
	public void initGUI() {
		...
		final JScrollPane scrollPane = new JScrollPane(drawPanel, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
		...
		drawPanel.setFocusTraversalKeys(KeyboardFocusManager.UP_CYCLE_TRAVERSAL_KEYS, keystrokes);
		toolBar.setFocusTraversalKeys(KeyboardFocusManager.UP_CYCLE_TRAVERSAL_KEYS, keystrokes);
	...
	/**
	 * Adds a new tool to the editor. This method is called during initialization
	 * and is used to add tools to the editor that where specified in the config.
	 * 
	...
	public final void setTool(DrawTool tool) {
		...
		if (currentTool != tool) {
			...
			view.repaint(); // necessary as the selection was cleared (i.e. the handles have been removed,
							// a repaint is necessary).
		}
		toolChanged();
	}
	...
	/**
	 * Close the window. This method will also dispose the frame and it will exit
	 * the JVM if it is the last open frame to be closed.
	 */
	private void close() {
		view.close();
		if (windowNr == 0) {
	...
	/**
	 * Retrieve the tool factories. This method can be used e.g. in the
	 * doPopulateTools method.
	 * 
